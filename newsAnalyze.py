...
print(data_title)  # 1000개의 title 문자열
print(data_description)  # 1000개의 description 문자열

# 제목과 요약 컬럼으로 dataframe 만들기
data_df = pd.DataFrame({"title":data_title, "description":data_description})
print(data_df)

### 데이터(텍스트) 전처리
# title 칼럼에서 한글을 제외한 문자 제거
data_df["title"] = data_df["title"].apply(lambda x : re.sub(r'[^ㄱ-ㅎ|가-힣]+'," ", x))
# description 칼럼에서 한글을 제외한 문자 제거
